...
function toneForCapabilities(capabilities) {
    if (capabilities?.dispatchable)
        return "green";
    if (capabilities?.terminal && capabilities?.retryable)
        return "red";
    if (capabilities?.retryable)
        return "yellow";
    return "gray";
}
...
export function CompilerControlPlanePage() {
    ...
    const updateGate = useCallback(async (gateId, state, metadata, risk) => {
        setLoading(true);
        setError(null);
        try {
