...
function Badge({ label, tone }: { label: string; tone: "green" | "yellow" | "gray" | "red" }) {
  const palette = {
    ...
    <AgencyLayout currentRoute="#/agency/step-library">
      ...
      <div style={{ display: "flex", gap: "10px", alignItems: "center", marginBottom: "14px", flexWrap: "wrap" }}>
        <input
          ...
          style={{ background: "#111", border: "1px solid #333", color: "#ddd", borderRadius: "6px", padding: "8px 10px", minWidth: "160px" }}
        />
        <input
          value={intentFilter}
          onChange={(event) => setIntentFilter(event.target.value)}
          placeholder="Filter intent"
          style={{ background: "#111", border: "1px solid #333", color: "#ddd", borderRadius: "6px", padding: "8px 10px", minWidth: "220px" }}
        />
        <button
      ...
      <div style={{ display: "grid", gridTemplateColumns: "minmax(400px, 1.1fr) minmax(360px, 0.9fr)", gap: "16px", alignItems: "start" }}>
        <div style={{ border: "1px solid #222", borderRadius: "6px", overflow: "hidden", background: "#0d0d0d" }}>
          ...
          {loading ? (
            ...
            <div style={{ padding: "32px", color: "#777", textAlign: "center" }}>No validated steps found.</div>
          ) : (
            entries.map((entry) => (
              <button
                key={entry.id}
              ...
              >
                <div style={{ minWidth: 0 }}>
                  <div style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", fontSize: "13px" }}>{entry.name}</div>
                  <div style={{ color: "#666", fontSize: "11px", marginTop: "4px" }}>
                    {entry.action ?? entry.type ?? "step"} · {entry.runIntent ?? "unknown intent"}
                  </div>
                </div>
        ...
        <div style={{ border: "1px solid #222", borderRadius: "6px", background: "#0d0d0d", padding: "16px" }}>
          {selected ? (
            <>
              ...
              <div style={{ border: "1px solid #222", borderRadius: "6px", padding: "12px", marginBottom: "12px", background: "#101010" }}>
                <div style={{ color: "#e5e7eb", fontSize: "13px", fontWeight: 600, marginBottom: "8px" }}>Contract</div>
                <div style={{ color: "#aaa", fontSize: "12px", marginBottom: "8px" }}>
                  Preconditions: {listPreview(selected.preconditions)}
                </div>
                <div style={{ color: "#aaa", fontSize: "12px" }}>
                  Postconditions: {listPreview(selected.postconditions)}
                </div>
              </div>

              <div style={{ border: "1px solid #222", borderRadius: "6px", padding: "12px", background: "#101010" }}>
                <div style={{ color: "#e5e7eb", fontSize: "13px", fontWeight: 600, marginBottom: "8px" }}>Evidence</div>
                <pre style={{ margin: 0, color: "#a1a1aa", fontSize: "11px", whiteSpace: "pre-wrap", overflowWrap: "anywhere", maxHeight: "220px", overflowY: "auto" }}>
                  {JSON.stringify(selected.evidence, null, 2)}
                </pre>
              </div>
            </>
          ) : (
            <div style={{ color: "#777", textAlign: "center", padding: "28px" }}>Select a step.</div>
          )}
        </div>
      </div>
    </AgencyLayout>
