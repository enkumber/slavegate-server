...
import { wireGuardService } from "../modules/wireguard/wireguard.service";

// List all WireGuard peers
router.get("/wireguard/peers", requireAuth, async (_req, res) => {
  try {
    const peers = await wireGuardService.list();
    res.json({ ok: true, data: peers });
  } catch (err) {
    console.error("[api] WireGuard list error:", (err as Error).message);
    res.status(500).json({ ok: false, error: (err as Error).message });
  }
});

// Provision new device with WireGuard peer
router.post("/devices/provision", requireAuth, async (req, res) => {
  const { name } = req.body as { name: string };
  if (!name || typeof name !== "string") {
    return res.status(400).json({ ok: false, error: "name (string) required" });
  }

  try {
    // 1. Create WireGuard peer
    console.log(`[api] Provisioning device: ${name}`);
    const peer = await wireGuardService.create(name);
    console.log(`[api] WireGuard peer created: ${peer.id} (${peer.address})`);

    // 2. Get config (with fixed public endpoint)
    const config = await wireGuardService.getConfig(peer.id);
    // QR will be generated on frontend from the fixed config
    const qrSvg = null;

    // 3. Create pending device in DB
    const db = getDb();
    const deviceResult = await db.query(
      `INSERT INTO devices (friendly_name, status, wireguard_peer_id, wireguard_ip)
       VALUES ($1, 'pending', $2, $3)
       RETURNING id`,
      [name, peer.id, peer.address]
    );
    const deviceId = deviceResult.rows[0].id as string;
    console.log(`[api] Device created: ${deviceId}`);
    ...
    res.json({
      ...
      data: {
        ...
        wireguardIp: peer.address,
        qrSvg,
        config,
      },
    });
  } catch (err) {
    console.error("[api] Provision error:", (err as Error).message);
    res.status(500).json({ ok: false, error: (err as Error).message });
  }
});

// Get QR code for existing device
router.get("/devices/:id/qr", requireAuth, async (req, res) => {
  try {
    const db = getDb();
    const result = await db.query(
      "SELECT wireguard_peer_id FROM devices WHERE id = $1",
      [req.params.id]
    );
    if (result.rows.length === 0) {
      return res.status(404).json({ ok: false, error: "Device not found" });
    }
    const peerId = result.rows[0].wireguard_peer_id as string | null;
    if (!peerId) {
      return res.status(400).json({ ok: false, error: "Device has no WireGuard peer" });
    }

    const qrSvg = await wireGuardService.getQR(peerId);
...
router.get("/devices/:id/wireguard-config", requireAuth, async (req, res) => {
  try {
    const db = getDb();
    const result = await db.query(
      "SELECT wireguard_peer_id FROM devices WHERE id = $1",
...
// Delete device and its WireGuard peer
router.delete("/devices/:id/full", requireAuth, async (req, res) => {
  try {
    const db = getDb();
    const result = await db.query(
      "SELECT wireguard_peer_id FROM devices WHERE id = $1",
    ...
    // Delete WireGuard peer first (if exists)
    if (peerId) {
      try {
        await wireGuardService.delete(peerId);
        console.log(`[api] WireGuard peer ${peerId} deleted`);
      } catch (err) {
        console.warn(`[api] Failed to delete WG peer: ${(err as Error).message}`);
      }
    }

    // Delete device from DB (cascade deletes related records)
    await devicesService.deleteDevice(req.params.id);
    console.log(`[api] Device ${req.params.id} deleted`);
...
router.get("/wireguard/health", requireAuth, async (_req, res) => {
