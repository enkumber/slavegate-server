...
describe("DB-authoritative workflow semantics", () => {
  ...
  it("stores migrated domain contracts and compiler policy in PostgreSQL", () => {
    const migration = fs.readFileSync(
      path.join(__dirname, "..", "..", "db", "migrations", "099_db_authoritative_workflow_semantics.sql"),
      "utf8",
    );

    expect(migration).toContain("INSERT INTO workflow_capabilities");
    expect(migration).toContain("'goalContract'");
    expect(migration).toContain("INSERT INTO system_prompts");
    expect(migration).toContain("runtime_semantic_entries");
    expect(migration).toContain("'vision_prompt'");
    expect(migration).toContain("'incident_routing_rule'");
    expect(migration).toContain("'account_detection_rule'");
    expect(migration).toContain("'tool_catalog'");
    expect(migration).toContain("ON CONFLICT (capability_key) DO UPDATE");
  });
});
