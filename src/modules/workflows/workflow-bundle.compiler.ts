...
const TECHNICAL_ACTIONS = new Set([
  "screen_wake", "screen_off", "unlock", "intent_send",
  "open_app", "close_app", "press_back", "press_home", "press_recent",
  "press_key", "keyevent", "screenshot", "screenshot_for_vlm",
  "ui_tree_dump", "get_screen_state", "get_foreground_app",
  "wait_for_idle", "detect_current_screen", "checkpoint",
]);

const VISIBLE_HUMAN_ACTIONS = new Set([
  "tap", "swipe", "scroll", "type_text", "set_focused_text",
  "a11y_find_tap", "ocr_find_tap", "long_press", "double_tap",
]);
...
const DEVICE_LOCAL_ACTIONS = new Set([
  ...TECHNICAL_ACTIONS,
  ...VISIBLE_HUMAN_ACTIONS,
  "set_variable", "classify_reddit_health_scan", "increment", "decrement",
  "reset_counter", "append_to_list", "mark_processed", "random_delay",
  "forced_pause", "branch_on_decision", "conditional_pause",
]);

const SERVER_ONLY_ACTIONS = new Set([
  "semantic_tap", "cascade_tap", "ensure_on_screen",
  "vlm_analyze_post_for_outreach", "vlm_generate_comment",
  "run_loop", "for_each",
]);
...
function isDeviceLocalStep(step: WorkflowStep): boolean {
  if (step.type === "action") {
    return DEVICE_LOCAL_ACTIONS.has(step.action) && !SERVER_ONLY_ACTIONS.has(step.action);
  }
  if (step.type === "condition") {
    return step.if_true.every(isDeviceLocalStep) && (step.if_false ?? []).every(isDeviceLocalStep);
  }
...
export function humanDelayForStep(
  ...
  return {
    type: "action",
    id: `${step.id ?? step.action}__human_delay`,
    action: "human_delay",
    timeoutMs: 4_000,
    params: {
      minMs: 750,
      ...
      reason: `social_visible_${step.action}`,
      requiredPackage,
    },
  };
}
...
function compileSteps(
  steps: WorkflowStep[],
  template: WorkflowTemplate,
  options: DeviceBundleCompileOptions,
): WorkflowStep[] {
  ...
  for (const step of steps) {
    if (step.type === "condition") {
      compiled.push({
        ...step,
        if_true: compileSteps(step.if_true, template, options),
...
export function compileDeviceWorkflowBundle(
  template: WorkflowTemplate,
  options: DeviceBundleCompileOptions,
): DeviceBundleTemplate {
  if (!canCompileDeviceBundle(template)) {
    throw new Error(`Workflow ${template.id} contains steps unsupported by device_bundle`);
  }
  const now = options.now ?? new Date();
  const deadlineAt = new Date(now.getTime() + (options.deadlineMs ?? 10 * 60_000)).toISOString();
