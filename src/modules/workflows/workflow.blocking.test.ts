...
describe("Generated workflow contract validation", () => {
  ...
  it("rejects generated workflow actions that can trigger root, VLM, file, or mutation paths", async () => {
    ...
    expect(result.errors).toContain(
      "workflow.steps[0].action must be one of: close_app, get_screen_state, open_app, press_key, screen_wake, screenshot, scroll, detect_current_screen, classify_reddit_health_scan, set_variable, swipe, ui_tree_dump, unlock, wait_for_idle"
    );
    expect(result.errors).toContain(
      "workflow.steps[1].action must be one of: close_app, get_screen_state, open_app, press_key, screen_wake, screenshot, scroll, detect_current_screen, classify_reddit_health_scan, set_variable, swipe, ui_tree_dump, unlock, wait_for_idle"
    );
    expect(result.errors).toContain(
      "workflow.steps[2].action must be one of: close_app, get_screen_state, open_app, press_key, screen_wake, screenshot, scroll, detect_current_screen, classify_reddit_health_scan, set_variable, swipe, ui_tree_dump, unlock, wait_for_idle"
    );
    expect(result.errors).toContain(
      "workflow.steps[3].action must be one of: close_app, get_screen_state, open_app, press_key, screen_wake, screenshot, scroll, detect_current_screen, classify_reddit_health_scan, set_variable, swipe, ui_tree_dump, unlock, wait_for_idle"
    );
  });
